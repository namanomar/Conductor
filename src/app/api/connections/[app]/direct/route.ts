...
export async function POST(req: Request, { params }: { params: Promise<{ app: string }> }) {
  ...
  try {
    if (app === "pagerduty") {
      ...
      await saveCredential(app, { type: "api_key", accessToken: body.apiKey, meta: { name: who.name } });
    ...
    if (app === "github") {
      ...
      await saveCredential(app, { type: "api_key", accessToken: body.token, meta: { login: who.login } });
    ...
    if (app === "slack") {
      ...
      await saveCredential(app, { type: "api_key", accessToken: body.token, meta: { team: who.team } });
